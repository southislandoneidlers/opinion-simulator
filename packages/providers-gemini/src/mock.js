...
exports.mockGeminiResult = mockGeminiResult;
function mockGeminiResult(input) {
    ...
    const result = {
        directReaction: "我先就現有材料回應：方向可以討論，但還看不到足夠的執行條件，所以只能有條件支持。",
        ...
        answers: [{ question: input.question, answer: "有條件支持，需補執行條件。" }],
        validationState: "valid"
    };
    return {
        result,
        rawResponse: { provider: "gemini", mode: "mocked", text: result.directReaction }
    };
}
